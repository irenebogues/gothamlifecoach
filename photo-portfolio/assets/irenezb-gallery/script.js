(function ($) {
    ...
    if (!isBuilder) {
        ...
        window.onYouTubeIframeAPIReady = function () {
            ...
            $('.video-slide').each(function (i) {
                ...
                $('.video-container').eq(i).append('<div id ="izb-video-' + i + '" class="izb-background-video" data-video-num="' + i + '"></div>')
                ...
                if ($(this).attr('data-video-url').indexOf('vimeo.com') != -1) {
                    ...
                    var player = new Vimeo.Player('izb-video-' + i, options);
                    player.playVideo = Vimeo.play;
                } else {
               
                var player = new YT.Player('izb-video-' + i, {
                    height: '100%',
                    width: '100%',
        ...
        function onPlayerReady(event) {
            if ($(event.target).closest('.izb-slider').hasClass('in')) {
    ...
    $window.on('show.bs.modal', function (e) {
        ...
        var timeout2 = setTimeout(function () {
            ...
            var slide = $(e.target).find('.carousel-item').eq(index).find('.izb-background-video');
            $(e.target).find('.carousel-item .izb-background-video')
            if (slide.length > 0) {
    ...
    $window.on('slide.bs.carousel', function (e) {
        var ytv = $(e.target).find('.carousel-item.active .izb-background-video');
        if (ytv.length > 0) {
